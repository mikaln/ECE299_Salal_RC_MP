...
vol_count = 0 # Increments to change volume
def check_button_to_change_vol(timer):
    if button_interrupt.Button_Active == True:
        global vol_count
        volume = vol_count % 16 # the parsed volume can only be between 0 and 15
        fm_radio.SetVolume( volume ) # Set volume
        fm_radio.ProgramRadio() # Program the radio with the new set volume
        vol_count = vol_count + 1 # Increment the volume count variable
        button_interrupt.Button_Active = False # Button has now been served -> set to false 
        print("Set volume to: %d " % volume)

# Set timer to change the volume if the button was pushed every 200ms        
Timer(mode = Timer.PERIODIC, period = 200, callback = check_button_to_change_vol )       
